        ...
        <div class="container">
            <ul>
                <?php 
                    $explodedString=[];
                    //ciclo le faq
                    foreach($faqContainer as $faq){
                        //printo il titolo
                        echo "<li>
                            <h2>{$faq['title']}</h2>
                        </li>";
                        //i punti e a capo li sostituisco con doppiopnto e li divito in array che poi andranno messi in paragrafi
                        $temp = explode("..", $faq['content']);
                        foreach($temp as $string){
                            $fadingString="";
                            for($i=0;$i<strlen($string);$i++){
                                //metto in paragrafi anche le stringhe che sono divise dai :
                                if($string[$i]==":"&&$string[$i+1]==":"){
                                    $explodedString[]=$fadingString.":";
                                    $fadingString="";
                                    $i++;
                                }else{
                                    $fadingString.=$string[$i];
                                }
                            }
                            //se la stringa della lavorazione è vuota e quindi sull'ultimo elemento non la pusho
                            if($fadingString!=""){
                                $explodedString[]=$fadingString;
                                $fadingString="";
                            }
                        }
                        //per ogni elemento negli array dei paragrafi(chiamato explodedString)
                        foreach($explodedString as $element){
                            $element=optimizeString($element);
                            echo "<p>{$element}</p>";
                        }
                        $explodedString=[];
                    }
